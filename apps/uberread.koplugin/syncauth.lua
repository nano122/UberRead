    ...
    if settings.refresh_token and settings.expires_at
        ...
        client:refresh_token(settings.refresh_token, function(success, response)
            if success then
                ...
                logger.err("UberReadSync: Token refresh failed:", response or "Unknown error")
            end
        end)
    end
end
...
function SyncAuth:getUberReadSyncClient(settings, path)
    if not settings.access_token or not settings.expires_at or settings.expires_at < os.time() then
        return nil
    end

    local UberReadSyncClient = require("uberreadsync")
    return UberReadSyncClient:new{
        service_spec = path .. "/uberread-sync-api.json",
...
function SyncAuth:doLogin(settings, path, email, password, menu)
    ...
    if success then
        ...
        UIManager:show(InfoMessage:new{
            text = _("Successfully logged in to UberRead"),
            timeout = 3,
        })
    else
        UIManager:show(InfoMessage:new{
...
function SyncAuth:logout(settings, path, menu)
    if settings.access_token then
        ...
        if client then
            client:sign_out(settings.access_token, function(success, _response)
                logger.dbg("UberReadSync: Sign out result:", success)
            end)
        end
    end
    ...
    UIManager:show(InfoMessage:new{
        text = _("Logged out from UberRead Sync"),
        timeout = 2,
    })
end
