...
local UPDATE_URLS = {
    "https://download.uberread.com/releases/latest.json",
    "https://github.com/uberread/uberread/releases/latest/download/latest.json",
}
local DOWNLOAD_URLS = {
    "https://download.uberread.com/releases/%s/UberRead-%s-1.koplugin.zip",
    "https://github.com/uberread/uberread/releases/download/%s/UberRead-%s-1.koplugin.zip",
}
...
function SelfUpdate:fetchLatestVersion()
    ...
    for _, url in ipairs(UPDATE_URLS) do
        ...
        logger.dbg("UberReadSync: failed to fetch latest.json from", url, "code:", code)
    end
    return nil
end
...
function SelfUpdate:downloadAndInstall(plugin_path, version)
    ...
    local tag = "v" .. version
    local zip_name = "UberRead-" .. version .. "-1.koplugin.zip"
    ...
    for _, url_template in ipairs(DOWNLOAD_URLS) do
        ...
        logger.dbg("UberReadSync: downloading from", url)
        ...
        logger.dbg("UberReadSync: download failed from", url, "code:", code)
    end
