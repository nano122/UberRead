...
function SyncConfig:getMetaHash(ui)
    local doc_uberread_sync = ui.doc_settings:readSetting("uberread_sync") or {}
    local meta_hash = doc_uberread_sync.meta_hash_v1
    if not meta_hash then
        meta_hash = self:generateMetadataHash(ui)
        doc_uberread_sync.meta_hash_v1 = meta_hash
        ui.doc_settings:saveSetting("uberread_sync", doc_uberread_sync)
    end
    return meta_hash
end
...
function SyncConfig:applyBookConfig(ui, config)
    logger.dbg("UberReadSync: Applying book config:", config)
