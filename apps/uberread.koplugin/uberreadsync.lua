...
local UberReadSyncClient = {
    service_spec = nil,
    access_token = nil,
}

function UberReadSyncClient:new(o)
    if o == nil then o = {} end
    setmetatable(o, self)
    self.__index = self
...
function UberReadSyncClient:init()
    local Spore = require("Spore")
    self.client = Spore.new_from_spec(self.service_spec)
    
    -- UberRead API headers middleware
    package.loaded["Spore.Middleware.UberReadHeaders"] = {}
    require("Spore.Middleware.UberReadHeaders").call = function(args, req)
        req.headers["content-type"] = "application/json"
        req.headers["accept"] = "application/json"
    end
    
    -- UberRead Bearer token auth middleware
    package.loaded["Spore.Middleware.UberReadAuth"] = {}
    require("Spore.Middleware.UberReadAuth").call = function(args, req)
        if self.access_token then
            req.headers["authorization"] = "Bearer " .. self.access_token
        else
            logger.err("UberReadSyncClient:access_token is not set, cannot authenticate")
            return false, "Access token is required for UberRead API"
        end
    end
...
function UberReadSyncClient:pullChanges(params, callback)
    self.client:reset_middlewares()
    self.client:enable("Format.JSON")
    self.client:enable("UberReadHeaders", {})
    self.client:enable("UberReadAuth", {})
    ...
    local co = coroutine.create(function()
        ...
        if ok then
            ...
            logger.dbg("UberReadSyncClient:pullChanges failure:", res)
...
function UberReadSyncClient:pushChanges(changes, callback)
    self.client:reset_middlewares()
    self.client:enable("Format.JSON")
    self.client:enable("UberReadHeaders", {})
    self.client:enable("UberReadAuth", {})
    ...
    local co = coroutine.create(function()
        ...
        if ok then
            ...
            logger.dbg("UberReadSyncClient:pushChanges failure:", res)
            callback(false, res.body)
        end
    end)
    self.client:enable("AsyncHTTP", {thread = co})
