...
-- KOReader color name → UberRead color value
local KO_TO_UBERREAD_COLOR = {
    yellow = "yellow",
    red = "red",
    green = "green",
    blue = "blue",
...
-- UberRead color value → KOReader color name
local UBERREAD_TO_KO_COLOR = {
    yellow = "yellow",
    red = "red",
    green = "green",
    blue = "blue",
    ...
    for _, item in ipairs(annotations) do
        ...
        if item.drawer and pos0 then
            ...
            table.insert(notes, {
                ...
                note = item.note or "",
                style = style,
                color = KO_TO_UBERREAD_COLOR[item.color or "yellow"],
                page = item.pageno,
...
function SyncAnnotations:push(ui, settings, client, interactive, full_sync)
    ...
    local meta_hash = ui.doc_settings:readSetting("uberread_sync") or {}
    ...
    logger.dbg("UberReadSync: Pushing annotations, payload:", payload)
    ...
    client:pushChanges(
        ...
        function(success, _response)
            ...
            if success then
                ...
                G_reader_settings:saveSetting("uberread_sync", settings)
            end
        end
    )
end
...
function SyncAnnotations:pull(ui, settings, client, book_hash, meta_hash, dialog, interactive, full_sync)
    ...
    client:pullChanges(
        ...
        function(success, response)
            ...
            logger.dbg("UberReadSync: Pulled annotations from sync:", #data)
            ...
            for _, note in ipairs(data) do
                ...
                if note_type == "bookmark" then
                    ...
                    item = {
                        ...
                        note = note.note or "",
                        drawer = drawer,
                        color = UBERREAD_TO_KO_COLOR[note.color] or "yellow",
                        pageno = pageno,
                        datetime = datetime_str,
                        datetime_updated = datetime_updated_str,
                    }
                ...
                logger.dbg("UberReadSync: Added annotation from sync:", item)
                added = added + 1
