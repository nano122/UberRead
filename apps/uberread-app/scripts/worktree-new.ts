...
const SKIPPED_SUBMODULES = [
  'apps/uberread-app/.claude/skills/gstack', // shared via .claude symlink
...
if (/^\d+$/.test(arg)) {
  ...
  localBranch = `pr-${arg}`;
  worktreePath = path.join(path.dirname(repoRoot), `uberread-${localBranch}`);
  ...
  localBranch = arg;
  worktreePath = path.join(path.dirname(repoRoot), `uberread-${arg.replace(/\//g, '-')}`);
...
const appRelPath = 'apps/uberread-app';
...
if (fs.existsSync(androidGenDir)) {
  ...
  execSync('pnpm tauri icon ../../data/icons/uberread-book.png', {
    stdio: gitStdio,
    cwd: dstAppDir,
  });
