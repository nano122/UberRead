...
pub fn run() {
    ...
    let builder = builder.plugin(
        tauri_plugin_single_instance::Builder::new()
            ...
            .dbus_id("com.bilingify.uberread".to_owned())
            .build(),
    );
    ...
    builder
        .setup(|#[allow(unused_variables)] app| {
            ...
            #[cfg(desktop)]
            let updater_disabled = std::env::var("UBERREAD_DISABLE_UPDATER").is_ok();
            #[cfg(not(desktop))]
            let updater_disabled = false;

            let init_script = format!(
                r#"
                    if ({is_eink}) window.__UBERREAD_IS_EINK = true;
                    if ({cli_access}) window.__UBERREAD_CLI_ACCESS = true;
                    if ({is_appimage}) window.__UBERREAD_IS_APPIMAGE = true;
                    if ({updater_disabled}) window.__UBERREAD_UPDATER_DISABLED = true;
