...
pub fn setup_macos_menu(app: &AppHandle) -> tauri::Result<()> {
    ...
    global_menu.append(
        &SubmenuBuilder::new(app, "Help")
            ...
            .text("uberread_help", "UberRead Help")
            .build()?,
    )?;
...
pub fn handle_menu_event(app: &AppHandle, event: &MenuEvent) {
    ...
    if event.id() == "open_file" {
        ...
        let _ = opener.open_url("https://uberread.com/privacy-policy", None::<&str>);
    } else if event.id() == "report_issue" {
        let _ = opener.open_url("https://github.com/uberread/uberread/issues", None::<&str>);
    } else if event.id() == "uberread_help" {
        let _ = opener.open_url("https://uberread.com/support", None::<&str>);
    }
}
