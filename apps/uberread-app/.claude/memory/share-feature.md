...
- **Universal Links / App Links**: handle `https://web.uberread.com/s/...` in v1 via `useOpenShareLink`. Tauri's `applinks` config already covers the host.
...
2. Migration directory: project has no `apps/uberread-app/supabase/migrations/`. Confirm where SQL actually lands before writing the file.
