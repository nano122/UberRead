...
declare global {
  interface Window {
    __UBERREAD_IS_EINK?: boolean;
    __UBERREAD_IS_APPIMAGE?: boolean;
    __UBERREAD_UPDATER_DISABLED?: boolean;
  }
}
...
const DIST_CHANNEL = (process.env['NEXT_PUBLIC_DIST_CHANNEL'] || 'uberread') as DistChannel;
...
export class NativeAppService extends BaseAppService {
  ...
  override isAppImage = Boolean(window.__UBERREAD_IS_APPIMAGE);
  override isEink = Boolean(window.__UBERREAD_IS_EINK);
  ...
  override hasUpdater =
    ...
    !window.__UBERREAD_UPDATER_DISABLED;
  ...
  async migrate20251029() {
    ...
    const oldDir = await join(rootPath, 'Images', 'UberRead', 'Images');

    await copyFiles(this, oldDir, newDir);

    const dirToDelete = await join(rootPath, 'Images', 'UberRead');
