...
const getPathResolver = ({ customRootDir }: { customRootDir?: string } = {}) => {
  ...
  const getCustomBasePrefix = isCustomBaseDir
    ...
    : undefined;
  const resolveFromCustom = (custom: string | undefined, ...parts: string[]) =>
    custom ? nodePath.join(custom, ...parts.filter(Boolean)) : undefined;
    ...
    switch (base) {
      case 'Settings':
        return {
          ...
          fp: resolveFromCustom(custom, fp) ?? fp,
          base,
        };
      case 'Cache':
        return {
      ...
      case 'Data':
        return {
          ...
          fp: resolveFromCustom(custom, DATA_SUBDIR, fp) ?? nodePath.join(DATA_SUBDIR, fp),
          base,
        };
      case 'Books':
        return {
          ...
          basePrefix: async () => custom ?? getAppDataDir(),
          fp:
            resolveFromCustom(custom, LOCAL_BOOKS_SUBDIR, fp) ??
      ...
      case 'Fonts':
        return {
          ...
          basePrefix: async () => custom ?? getAppDataDir(),
          fp:
            resolveFromCustom(custom, LOCAL_FONTS_SUBDIR, fp) ??
      ...
      case 'Images':
        return {
          ...
          basePrefix: async () => custom ?? getAppDataDir(),
          fp:
            resolveFromCustom(custom, LOCAL_IMAGES_SUBDIR, fp) ??
      ...
      case 'Dictionaries':
        return {
          ...
          basePrefix: async () => custom ?? getAppDataDir(),
          fp:
            resolveFromCustom(custom, LOCAL_DICTIONARIES_SUBDIR, fp) ??
