...
vi.mock('@/services/constants', () => ({
  UBERREAD_WEB_BASE_URL: 'https://web.uberread.com',
  UBERREAD_NODE_BASE_URL: 'https://node.uberread.com',
}));
...
beforeEach(() => {
  ...
  delete (window as unknown as Record<string, unknown>)['__UBERREAD_CLI_ACCESS'];
});
...
describe('environment', () => {
  ...
  // ── hasCli ─────────────────────────────────────────────────────
  describe('hasCli', () => {
    test('returns true when __UBERREAD_CLI_ACCESS is true', async () => {
      window.__UBERREAD_CLI_ACCESS = true;
      const { hasCli } = await import('@/services/environment');
      expect(hasCli()).toBe(true);
    });

    test('returns false when __UBERREAD_CLI_ACCESS is not set', async () => {
      const { hasCli } = await import('@/services/environment');
      expect(hasCli()).toBe(false);
    });

    test('returns false when __UBERREAD_CLI_ACCESS is explicitly false', async () => {
      window.__UBERREAD_CLI_ACCESS = false;
      const { hasCli } = await import('@/services/environment');
      expect(hasCli()).toBe(false);
    });
  });
  ...
  describe('getBaseUrl', () => {
    ...
    test('falls back to UBERREAD_WEB_BASE_URL when env var not set', async () => {
      delete env['NEXT_PUBLIC_API_BASE_URL'];
      const { getBaseUrl } = await import('@/services/environment');
      expect(getBaseUrl()).toBe('https://web.uberread.com');
    });
  });
  ...
  describe('getNodeBaseUrl', () => {
    ...
    test('falls back to UBERREAD_NODE_BASE_URL when env var not set', async () => {
      delete env['NEXT_PUBLIC_NODE_BASE_URL'];
      const { getNodeBaseUrl } = await import('@/services/environment');
      expect(getNodeBaseUrl()).toBe('https://node.uberread.com');
    });
  });
  ...
  describe('getAPIBaseUrl', () => {
    ...
    test('returns full URL in production', async () => {
      ...
      delete env['NEXT_PUBLIC_API_BASE_URL'];
      const { getAPIBaseUrl } = await import('@/services/environment');
      expect(getAPIBaseUrl()).toBe('https://web.uberread.com/api');
    });
  ...
  describe('getNodeAPIBaseUrl', () => {
    ...
    test('returns full node URL in production', async () => {
      ...
      delete env['NEXT_PUBLIC_NODE_BASE_URL'];
      const { getNodeAPIBaseUrl } = await import('@/services/environment');
      expect(getNodeAPIBaseUrl()).toBe('https://node.uberread.com/api');
    });
