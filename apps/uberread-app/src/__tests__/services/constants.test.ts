...
import {
  ...
  CJK_FONTS_PATTENS,
  BOOK_IDS_SEPARATOR,
  DOWNLOAD_UBERREAD_URL,
  UBERREAD_WEB_BASE_URL,
  UBERREAD_NODE_BASE_URL,
  UBERREAD_UPDATER_FILE,
  UBERREAD_CHANGELOG_FILE,
  UBERREAD_PUBLIC_STORAGE_BASE_URL,
  UBERREAD_OPDS_USER_AGENT,
...
describe('services/constants', () => {
  ...
  describe('URL constants', () => {
    ...
    it('DOWNLOAD_UBERREAD_URL is a valid URL', () => {
      expect(DOWNLOAD_UBERREAD_URL).toMatch(/^https:\/\//);
    });

    it('UBERREAD_WEB_BASE_URL is a valid URL', () => {
      expect(UBERREAD_WEB_BASE_URL).toMatch(/^https:\/\//);
    });

    it('UBERREAD_NODE_BASE_URL is a valid URL', () => {
      expect(UBERREAD_NODE_BASE_URL).toMatch(/^https:\/\//);
    });

    it('UBERREAD_UPDATER_FILE is a URL ending with .json', () => {
      expect(UBERREAD_UPDATER_FILE).toMatch(/^https:\/\//);
      expect(UBERREAD_UPDATER_FILE).toMatch(/\.json$/);
    });

    it('UBERREAD_CHANGELOG_FILE is a URL ending with .json', () => {
      expect(UBERREAD_CHANGELOG_FILE).toMatch(/^https:\/\//);
      expect(UBERREAD_CHANGELOG_FILE).toMatch(/\.json$/);
    });

    it('UBERREAD_PUBLIC_STORAGE_BASE_URL is a valid URL', () => {
      expect(UBERREAD_PUBLIC_STORAGE_BASE_URL).toMatch(/^https:\/\//);
    });

    it('UBERREAD_OPDS_USER_AGENT is a non-empty string', () => {
      expect(typeof UBERREAD_OPDS_USER_AGENT).toBe('string');
      expect(UBERREAD_OPDS_USER_AGENT.length).toBeGreaterThan(0);
    });
  });
