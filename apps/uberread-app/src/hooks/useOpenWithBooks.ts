...
/**
 * Handle "Open with UberRead" file imports. Consumes the `app-incoming-url`
 ...
 *
 * Non-file URL shapes (https, uberread://, data:, blob:) are skipped here
  ...
  useEffect(() => {
    ...
    const handle = async (urls: string[]) => {
      ...
      for (let url of urls) {
        ...
        if (!/^(https?:|data:|blob:|uberread:)/i.test(url)) {
          filePaths.push(url);
        }
      }
