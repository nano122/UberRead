...
import { DOWNLOAD_UBERREAD_URL } from '@/services/constants';
...
const SettingsMenu: React.FC<SettingsMenuProps> = ({ onPullLibrary, setIsDropdownOpen }) => {
  ...
  const showAboutUberRead = () => {
    setAboutDialogVisible(true);
    setIsDropdownOpen?.(false);
  };

  const downloadUberRead = () => {
    window.open(DOWNLOAD_UBERREAD_URL, '_blank');
    setIsDropdownOpen?.(false);
  };
  ...
  const handleSetSavedBookCoverForLockScreen = async () => {
    if (!(await requestStoragePermission()) && appService?.distChannel === 'uberread') return;
  ...
  return (
    ...
    >
      ...
      {user && userProfilePlan === 'free' && (
        <MenuItem label={_('Upgrade to UberRead Premium')} onClick={handleUpgrade} />
      )}
      {isWebAppPlatform() && <MenuItem label={_('Download UberRead')} onClick={downloadUberRead} />}
      <MenuItem label={_('About UberRead')} onClick={showAboutUberRead} />
      <MenuItem
        label={_('Help improve UberRead')}
