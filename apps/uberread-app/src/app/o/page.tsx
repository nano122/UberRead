...
import { DOWNLOAD_UBERREAD_URL, UBERREAD_WEB_BASE_URL } from '@/services/constants';
...
const ANDROID_PACKAGE = 'com.bilingify.uberread';
const FALLBACK_TIMEOUT_MS = 1500;
const DESKTOP_FALLBACK_DELAY_MS = 1000;

const buildIntentUrl = (path: string, fallbackUrl: string) => {
  const cleanPath = path.replace(/^\//, '');
  return `intent://${cleanPath}#Intent;scheme=uberread;package=${ANDROID_PACKAGE};S.browser_fallback_url=${encodeURIComponent(fallbackUrl)};end`;
};
...
const OpenAnnotationLanding = () => {
  ...
  useEffect(() => {
    ...
    if (platform === 'android-chromium') {
      const absoluteFallback = `${UBERREAD_WEB_BASE_URL}${webReaderUrl}`;
      ...
      <main className='bg-base-200 flex min-h-dvh flex-col items-center justify-center p-4 sm:p-8'>
        <Card>
          <div className='flex flex-col items-center text-center'>
            ...
            <a href='https://uberread.com' className='btn btn-ghost btn-block mt-6' rel='noopener'>
              {_('Go to UberRead')}
            </a>
          </div>
        </Card>
  ...
  return (
    <main className='bg-base-200 flex min-h-dvh flex-col items-center justify-center p-4 sm:p-8'>
      <Card>
        <BrandHeader
          title={_('Open in UberRead')}
          subtitle={
            showManualOpen
              ? _("If UberRead didn't open automatically, choose an option below:")
        ...
        {!showManualOpen && (
          ...
          >
            ...
            <span className='text-base-content/70 text-sm'>{_('Opening UberRead...')}</span>
          </div>
        )}
        ...
        >
          ...
          <p className='text-base-content/60 mt-3 text-center text-xs'>
            {_("Don't have UberRead?")}{' '}
            <a
              href={DOWNLOAD_UBERREAD_URL}
              target='_blank'
              rel='noopener'
