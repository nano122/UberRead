import { UBERREAD_WEB_BASE_URL, SHARE_BASE_URL, SHARE_TOKEN_LENGTH } from '@/services/constants';
...
//   uberread://share/{token}
//   https://web.uberread.com/s/{token}
...
export const parseShareDeepLink = (url: string): ShareDeepLink | null => {
  ...
  if (parsed.protocol === 'uberread:') {
    // For uberread://share/{token} the host portion holds the path segment
  ...
  if (parsed.protocol === 'https:' || parsed.protocol === 'http:') {
    if (!isWebUberReadHost(parsed.host)) return null;
...
const isWebUberReadHost = (host: string): boolean => {
  // Matches the production host and any preview domain UberRead may serve from.
  // Conservative: accepts only the exact production host or a *.uberread.com
  // subdomain so a third-party site cannot impersonate a share URL.
  if (host === new URL(UBERREAD_WEB_BASE_URL).host) return true;
  return host.endsWith('.uberread.com');
};
