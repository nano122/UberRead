import { UBERREAD_WEB_BASE_URL } from '@/services/constants';
 ...
 * and Readwise sync. Mobile App Links (web.uberread.com) intercept this URL
...
export const buildAnnotationWebUrl = ({ bookHash, noteId, cfi }: AnnotationDeepLink): string => {
  const base = `${UBERREAD_WEB_BASE_URL}${ANNOTATION_PATH_PREFIX}${bookHash}/annotation/${noteId}`;
...
export const buildAnnotationAppUrl = ({ bookHash, noteId, cfi }: AnnotationDeepLink): string => {
  const base = `uberread://book/${bookHash}/annotation/${noteId}`;
  return cfi ? `${base}?cfi=${encodeURIComponent(cfi)}` : base;
};

/**
 * Parse an incoming uberread:// or https://web.uberread.com annotation URL.
...
export const parseAnnotationDeepLink = (url: string): AnnotationDeepLink | null => {
  ...
  const isCustomScheme = parsed.protocol === 'uberread:';
  const isWebHost =
    (parsed.protocol === 'https:' || parsed.protocol === 'http:') &&
    parsed.host === 'web.uberread.com';
  if (!isCustomScheme && !isWebHost) return null;

  // For uberread:// URLs the URL parser stores the first path segment in the
