...
import { UBERREAD_UPDATER_FILE, UBERREAD_CHANGELOG_FILE } from '@/services/constants';
...
export const UpdaterContent = ({
  ...
  useEffect(() => {
    ...
    const checkAndroidUpdate = async () => {
      ...
      const response = await fetch(UBERREAD_UPDATER_FILE);
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const apkFilePath = await appService.resolveFilePath(
          `UberRead_${data.version}_${arch}.apk`,
          'Cache',
        );
        setUpdate({
          currentVersion,
    ...
    const checkWindowsPortableUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const exeFileName = `UberRead_${data.version}_${arch}-portable.exe`;
        ...
        setUpdate({
          ...
          downloadAndInstall: async (onEvent) => {
            ...
            try {
              ...
              const command = Command.create('start-uberread', ['/C', 'start', '', exeFilePath]);
              await command.spawn();
    ...
    const checkAppImageUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const downloadUrl = data.platforms[platformKey]?.url as string;
        const appImageFileName = `UberRead_${data.version}_${arch}.AppImage`;
  ...
  useEffect(() => {
    const fetchChangelogs = async (fromVersion: string): Promise<Changelog[]> => {
      try {
        const fetch = isTauriAppPlatform() ? tauriFetch : window.fetch;
        const res = await fetch(UBERREAD_CHANGELOG_FILE);
          ...
          {checkUpdate ? (
            <div className='text-base-content flex-grow text-sm'>
              <h2 className='mb-4 text-center font-bold sm:text-start'>
                {_('A new version of UberRead is available!')}
              </h2>
              <p className='mb-2'>
                {_('UberRead {{newVersion}} is available (installed version {{currentVersion}}).', {
                  newVersion,
                  currentVersion,
                })}
              </p>
                ...
                {changelogs.length > 0 && semver.gt(changelogs[0]!.version, currentVersion) ? (
                  <div className='flex gap-2'>
                    ...
                    {appService?.isAndroidApp && (
                      <Link
                        href='https://play.google.com/store/apps/details?id=com.bilingify.uberread'
                        target='_blank'
                        rel='noopener noreferrer'
                        className='btn btn-primary btn-sm'
                      >
...
export const UpdaterWindow = () => {
  ...
  return (
    <Dialog
      id='updater_window'
      isOpen={isOpen}
      title={checkUpdate ? _('Software Update') : _("What's New in UberRead")}
